...
baseline_z = 0.205 #0.15 #0.205

acoustic_thres = 0 #85 #85 #85 #80 #70 #80 #75
...
class AutoCalibration():
    def __init__(self, images, images_depth, sonars, masks, init_params, config, max_iters = 100, gt_params=None,  params_grid=None):
        ...
        for image in images_depth:
            ...
            image = cv2.medianBlur(image, 9) #31)
            # image = cv2.convertScaleAbs(image, alpha=2) #1.5)
        ...
        self.gamma_trans_u_ = 0.05 #0.07 #0.1 #0.07 #0.03 #0.03 #0.03 #0.05 #0.06 #0.1 #0.1 # upper bound
        self.gamma_trans_l_ = 0.001 #0.001 #0.01 #0.001 # lower bound
        
        self.gamma_rot_ = 0.001 # rotation
        self.gamma_rot_u_ = 0.03 #0.05 #0.02 #0.02 #0.01 #0.02 #0.05 #3.14 * np.pi/180 #0.05 #3.14 * np.pi/180 #0.05 # upper bound
        ...
        ### misc
        self.MAX_BINS = 256 #256 #256
        self.num_bins = 256 #256 #256
    ...
    def extract_sonar_pointcloud(self, sonar_im):
        # sonar_im_ = cv2.cvtCxolor(sonar_im.copy(), cv2.COLOR_GRAY2BGR)
        ...
        for i in range(self.N):
            ...
            for group_idx in range(int(p_C_intensity_masked.shape[1] / (phi_max*2+1))):
                if not np.isnan(p_C_intensity_masked_groups[group_idx,:,:]).any():
                    ...
                    gray = np.array(self.images[i][group[1,:].astype(int), group[0,:].astype(int)])
            ...
            try:
                ...
                mask = np.abs(gray_masked_bin - gray_masked_bin.mean()) < 2.6 * gray_masked_bin.std() #1.5
                gray_masked_bin = gray_masked_bin[mask] 
                intensity_masked_bin = intensity_masked_bin[mask]
                # breakpoint()
                self.tmp = np.append(self.tmp, np.unique(gray_masked_bin))
    ...
    def optimize(self):
        ...
        for idx in range(self.max_iters_):
            ...
            print(f"[iter={idx}] f = {f} - ext_vec = {ext_vec * [1, 1, 1, 1/D2R, 1/D2R, 1/D2R] }")
