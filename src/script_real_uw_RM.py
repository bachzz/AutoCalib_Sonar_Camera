import cv2
import numpy as np
from auto_calibration_real_uw_RM import AutoCalibration
...
for pair in zip(init_ranges['x'], init_ranges['yaw']):
    ...
    calib = AutoCalibration(images, sonars, masks, init_params, config, gt_params=gt_params, params_grid=params_grid)
    ...
    fid = 6 #0 #6 #0 #6
    image_init = calib.project_sonar_pointcloud_to_image(ext_vec_init, sonars[fid], cv2.convertScaleAbs(cv2.cvtColor(calib.images_RM[fid], cv2.COLOR_GRAY2BGR), alpha=1, beta=1 ))
