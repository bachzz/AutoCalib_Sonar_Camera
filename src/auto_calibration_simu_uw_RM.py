...
class AutoCalibration():
    def __init__(self, images, sonars, masks, init_params, config, max_iters = 100, gt_params=None,  params_grid=None):
        ...
        for image in images: #images_depth: #images:
            ...
            RM = np.abs(R_channel - M_channel).astype(np.uint8)
    ...
    def plot_inliers_scores(self, flag):  
        ...
        def compute_MI_lambda(x, y, flag):
            flag_idxes = np.where(np.array(list(flag)) == '1')[0]
    ...
    def compute_hist(self, ext_vec):
        ...
        for i in range(self.N):
            ...
            try:
                ...
                ## remove outliers
                mask = np.abs(gray_masked_bin - gray_masked_bin.mean()) < 3 * gray_masked_bin.std()
